    ...
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <Link href="/contact" className="px-5 py-2 text-xs font-bold rounded-full transition-all border border-white text-white hover:bg-white hover:text-kmtb-navy shadow-lg">
            HUBUNGI KAMI
          </Link>
        </nav>
      ...
      {/* MOBILE MENU */}
      <div className={`md:hidden absolute w-full bg-[#202E50] border-t border-white/10 transition-all duration-300 ${isMobileMenuOpen ? "max-h-screen opacity-100" : "max-h-0 opacity-0 overflow-hidden"}`}>
        <div className="flex flex-col p-6 space-y-4">
          {navLinks.map((link) => (
            <Link key={link.name} href={link.href} onClick={() => setIsMobileMenuOpen(false)} className="text-white font-bold text-sm uppercase border-b border-white/10 pb-2">
              {link.name}
            </Link>
          ))}
        </div>
